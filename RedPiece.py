...
def possible_red_piece_moves(current_x, current_y):
    """
    ...
    if CheckersBoard.checkers_board[current_x][current_y][1] == "K":
        if current_piece[0] == "B":
            red_piece = BlackKingPiece.black_king_piece
            current_piece = CheckersBoard.checkers_board[current_x][current_y][0] + "K"
        else:
            red_piece = RedKingPiece.red_king_piece
            current_piece = CheckersBoard.checkers_board[current_x][current_y][0] + "K"
    else:
        red_piece = RedPiece(current_x, current_y, current_piece)

    if red_piece not in CheckersPieces.possible_moves:
        CheckersPieces.possible_moves[red_piece] = []

    if current_x + 1 < 8 and current_y - 1 >= 0 and not can_capture_right(current_x, current_y,
                                                                          current_piece) and not BlackPiece.red_king_capture_check(
        red_piece):
        if CheckersBoard.checkers_board[current_x + 1][current_y - 1] == "  ":
            valid_move = (current_x + 1, current_y - 1)
            CheckersPieces.possible_moves[red_piece].append(valid_move)

    if current_x + 1 < 8 and current_y + 1 < 8 and not can_capture_left(current_x, current_y,
                                                                        current_piece) and not BlackPiece.red_king_capture_check(
        red_piece):
        if CheckersBoard.checkers_board[current_x + 1][current_y + 1] == "  ":
            valid_move = (current_x + 1, current_y + 1)
    ...
    possible_red_piece_moves_left(current_x, current_y, current_piece, red_piece)
    possible_red_piece_moves_right(current_x, current_y, current_piece, red_piece)
    del_no_capture_moves()


def possible_red_piece_moves_left(current_x, current_y, current_piece, red_piece):
    """
    Checks to see if piece can capture on the left side
    :param current_x: The "row" of the piece via the 2D array checkers board
    :param current_y: The "column" of the piece via the 2D array checkers board
    :param current_piece: The specific piece
    :param red_piece: A piece object
    :return: None
    """
...
def possible_red_piece_moves_right(current_x, current_y, current_piece, red_piece):
    """
...
def can_capture_left(current_x, current_y, current_piece):
    ...
    """
    # left side diagonal
    if current_x + 1 >= 8 or current_y - 1 < 0:
        return False

    if current_x + 2 >= 8 or current_y - 2 < 0:
        return False
...
def can_capture_right(current_x, current_y, current_piece):
    ...
    """
    # right side diagonal
    if current_x + 1 >= 8 or current_y + 1 > 8:
        return False

    if current_x + 2 >= 8 or current_y + 2 >= 8:
        return False
...
def del_no_capture_moves():
    ...
    for pieces in CheckersPieces.possible_moves:
        if pieces.can_capture and pieces.piece[0] == "R":
            capture_ctr += 1

    if capture_ctr > 0:
        for pieces in CheckersPieces.possible_moves:
            if not pieces.can_capture and pieces.piece[0] == "R":
                CheckersPieces.possible_moves[pieces] = []


def black_king_capture_check(piece):
    """
    Helps with black king if piece can be captured
